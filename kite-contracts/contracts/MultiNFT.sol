...
contract MultiCollectionNFT is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable {

       Kite public kiteContract; 
    ...
    function initialize() public initializer {
        __ERC721_init("Kite","kITE");
        __Ownable_init(msg.sender);
              kiteContract = Kite(address(0x5c7E626340B4f821FD22Adf433ff2e3fa6f9CF30));
    }

    // Mint a new NFT and assign it to a specific collection
    function mint(address to, uint256 tokenId, uint256 collectionId) internal onlyOwner {
    ...
    function claim(uint256 tokenId) external {
        ...
        // Check if the user has an active pool in the Kite contract
        address userPool = kiteContract.getPoolByToken(user);
        require(userPool != address(0), "User has no active pool in Kite");
    ...
    function getCollectionHolders(uint256 collectionId) external view returns (address[] memory) {
  ...
  // Function to set conditions for an existing collection
    function setConditions(uint256 collectionId, string calldata conditions) external onlyOwner {
        require(collectionId > 0, "Collection ID must be greater than zero");
